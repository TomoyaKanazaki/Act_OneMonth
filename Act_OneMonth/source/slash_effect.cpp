...
HRESULT CSlash_Effect::Init(void)
{
	...
	//使用するテクスチャを設定
	int tex = CTexture::SLASH_01 + (rand() % NUM_TEXTURE);

	// テクスチャ割り当て
	BindTexture(CManager::GetInstance()->CManager::GetInstance()->GetInstance()->GetTexture()->GetAddress(tex));
...
//  生成処理
//==========================================
CSlash_Effect* CSlash_Effect::Create(const D3DXVECTOR3& pos, const D3DXVECTOR3& rot)
{
	// インスタンス生成
